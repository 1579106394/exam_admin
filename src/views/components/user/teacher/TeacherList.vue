<template>
  <div>
      ...
      <el-form :rules="rules" ref="teacher" :inline="true" :model="teacher" label-width="80px" size="mini">
        ...
        <el-form-item label="职称" prop="teacherCollege">
          <el-select v-model="teacher.teacherTitle" filterable placeholder="请选择" clearable>
            <el-option v-for="title in titleList" :key="title.dictId" :label="title.dictName" :value="title.dictId">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="职务">
          <el-select v-model="teacher.teacherJob" filterable placeholder="请选择" clearable>
            <el-option v-for="job in jobList" :key="job.dictId" :label="job.dictName" :value="job.dictId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="入职时间" prop="teacherName">
          <el-date-picker placeholder="选择时间" v-model="teacher.teacherEntryTime" value-format="yyyy-MM-dd"
    ...
    <el-dialog title="详细信息" :visible.sync="resumeVisible" v-loading="loading">
      <el-form ref="teacher" :inline="true" :model="teacher" label-width="80px" size="mini">
    ...
    <el-dialog title="修改角色" :visible.sync="dialogRoleTree">
      <el-tree :data="roleTree" :props="defaultProps" show-checkbox node-key="roleId" default-expand-all
        v-loading="loading" :expand-on-click-node="false" :default-checked-keys="checkIds"
  ...
  export default {
    ...
    methods: {
        ...
        TeacherRoleApi.roleList(id).then(res => {
          if (res.code == 200) {
            ...
            this.checkIds.forEach((value, key) => {
              console.log(value);
              this.teacherRoles.push({ "trTeacher": this.checkTeacher, "trRole": value })
            })
          }
        })
      },
      ...
      handleCheckChange(data, checked, indeterminate) {
        ...
        if (checked && index === -1) {
          this.teacherRoles.push({ "trTeacher": this.checkTeacher, "trRole": data.roleId })
        }
